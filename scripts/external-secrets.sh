...
# EXTERNALSECRETS_REPO_SPEC=$(cat ../apps/external-secrets/base/helmrepository.yaml)
# EXTERNALSECRETS_REPO_URL=$(echo "$EXTERNALSECRETS_REPO_SPEC" | yq eval '.spec.url' -)
# EXTERNALSECRETS_RELEASE_SPEC=$(kustomize build ../apps/external-secrets/dev | yq 'select(.kind == "HelmRelease")')
...
# kubectl apply -f ../apps/external-secrets-config/base/bitwarden-self-signed-cert.yaml
